...
class FSMSubscribe(StatesGroup):
    pass
...
async def command_start(message: types.Message):
    ...
    with Session() as session:
        ...
        subscribes = {category.id: category.last_id for category in session.query(
            Category).filter(Category.id.in_(categories_to_subscribe)).all()}
        if user:
...
async def main():
    ...
    with Session() as session:
        ...
        categories = session.query(
            Category.id, Category.name).order_by(Category.id).all()
